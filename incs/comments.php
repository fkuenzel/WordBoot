...
function bs4_comment_form( $args ) {
	...
	$args['fields'] = array(
		...
		<div class="form-group comment-form-email">
			<label for="email" >' . __( 'E-Mail', 'bs4_lang' ) .
			( $req ? ' <span class="required text-danger">*</span>' : '' ) . '</label>
		...
		<div class="form-group comment-form-url">
			<label for="url" >' . __( 'Webseite', 'bs4_lang' ) . '</label>
    ...
    $args['comment_field'] = '
		<div class="form-group comment-form-comment">
			<label for="comment" >' . __( 'Kommentar', 'bs4_lang' ) . '</label>
			<textarea class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea>
		</div>';    
	$args['comment_notes_before'] = '<div class="alert alert-info">'.  __ ('Deine Email-Adresse wird nicht veröffentlicht.', 'bs4_lang') .'</div>';
	$args['comment_notes_after'] = '<div class="alert alert-info form-allowed-tags">' . sprintf( __( 'Sie können diese <abbr title="HyperText Markup Language">HTML</abbr> Tags und Attribute verwenden: %s', 'bs4_lang' ), ' <code>' . allowed_tags() . '</code>' ) . '</div>';
...
function bs4_comments_navigation( $args = array() ) {
    ...
    if ( get_comment_pages_count() > 1 ) {
        $args = wp_parse_args( $args, array(
            'prev_text'          => __( '<span aria-hidden="true">&larr;</span> Ältere Kommentare', 'bs4_lang' ),
            'next_text'          => __( 'Neuere Kommentare <span aria-hidden="true">&rarr;</span>', 'bs4_lang' ),
            'screen_reader_text' => __( 'Kommentar Navigation', 'bs4_lang' ),
        ) );
...
function bs4_comments_alert() {
	...
	if ( is_page() && ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments') ) {
		$output .= "<div class='alert alert-danger mt-3'>\n
		<i class='fa fa-exclamation-triangle'></i> <span class='h4'>". __( 'Kommentare sind für diese Seite geschlossen.', 'bs4_lang' ) ."</span>\n
		</div>\n";
	} else if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
		$output .= "<div class='alert alert-danger mt-3'>\n
		<i class='fa fa-exclamation-triangle'></i> <span class='h4'>". __( 'Kommentare sind für diesen Artikel geschlossen.', 'bs4_lang' ) ."</span>\n
		</div>\n";
	}
...
?>
	...
	<header class="comment-head list-group-item-heading">
		...
		<?php endif; ?>
		<?php printf( __( '<cite class="fn">%s</cite> <span class="says">sagt:</span>', 'bs4_lang' ), get_comment_author_link() ); ?>
		</div>
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em class="comment-awaiting-moderation text-info"><?php _e( 'Ihr Beitrag wartet auf die Moderation.', 'bs4_lang' ); ?></em>
			<br />
		<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				...
				printf( __('%1$s um %2$s','bs4_lang'), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Bearbeiten)', 'bs4_lang' ) );
			?>
		</div>
	</header>
